...
function getColor_Parcels(d) {
    return d == "Землі житлової та громадської забудови" ? '#cab562' :
    d == "Землі промисловості, транспорту, зв’язку, енергетики, оборони та іншого призначення" ? '#9345af' :
    d == "Землі рекреаційного призначення" ? 'green' :
	d == "Землі сільськогосподарського призначення" ? 'lightblue' :
    d == "Інше" ? '#8695ad' : '#8695ad'
}

function style_Parcels(feature) {
    return {
        color: getColor_Parcels(feature.properties.category),
        fillOpacity: 0.3,
        weight: 1,
    };
}
...
var baseTree = {
    ...
    children: [{
            label: googlemaps.options.layername,
            layer: googlemaps
        }, {
            label: openstreetmap.options.layername,
            layer: openstreetmap
        }, {
            label: arcgis_map.options.layername,
            layer: arcgis_map
        },
		{
            label: 'Генплан - основне креслення',
            layer: L.tileLayer('https://brych92.github.io/maps/Основне креслення/{z}/{x}/{y}.png', {
                maxNativeZoom: 17
            })
        }, {
            label: 'План зонування',
            layer: L.tileLayer('https://brych92.github.io/maps/Зонування/{z}/{x}/{y}.png', {
                maxNativeZoom: 17
            })
        }, {
            label: 'ІАОП',
            layer: L.tileLayer('https://brych92.github.io/maps/ІАОП/{z}/{x}/{y}.png', {
                maxNativeZoom: 18
            })
        }
...
var overlaysTree = {
    ...
    children: [{
            label: Parcels.options.layername,
            layer: Parcels
        }, {
            label: '<div class="leaflet-control-layers-separator"></div>'
        }, {
            label: LayerDrawnByGeoman.options.layername,
            layer: LayerDrawnByGeoman
        }, {
            label: '<div class="leaflet-control-layers-separator"></div>'
        }
    ]
...
function save_layer(layer_id, layer_name) {
    ...
    var file = new File([JSON.stringify(collection)], layer_name + '.geojson', {
...
function change_geoman_map_events() {
    edit_layers_id_list = get_edit_layers_id_list()
        ...
        map.pm.setGlobalOptions({
            ...
            panes: {
                vertexPane: 'Geoman_marker',
                layerPane: 'Geoman',
                markerPane: 'Geoman_marker'
            }
        })
        ...
        map.pm.setPathOptions({
            ...
            weight: 7,
            stripeColor: 'black',
            icon: orangeIcon, /*renderer: L.canvas(), */ stripeId: "diagonal_left_down_to_right_up"
        }, {
            merge: true,
            ignoreShapes: ['Line'],
        });
...
map.addControl(new information_control_container({
        position: "bottomleft"
    }));
...
map.addControl(new legend_control_container({
...
map.addControl(new Geoman_label({
        position: "topleft"
    }));
...
function set_legend_select() {
    ...
    var legend_selector = document.getElementById("legend_selector")
        ...
        var legend_selector_options = legend_selector.options
            ...
            case 'Parcels':
			"Землі житлової та громадської забудови" ? '#cab562' :
    d == "Землі промисловості, транспорту, зв’язку, енергетики, оборони та іншого призначення" ? 'purple' :
    d == "Землі рекреаційного призначення" ? 'green' :
	d == "Землі сільськогосподарського призначення" ? 'grey' :
    d == "Інше"
                var ownership_form_list = ["Землі житлової та громадської забудови", "Пемлі промисловості, транспорту, зв’язку, енергетики, оборони та іншого призначення", "Землі рекреаційного призначення", "Землі сільськогосподарського призначення","Інше"]
                legend_container_content.innerHTML = `За категорією:`
                ...
                <th style="text-align: left; text-decoration: none">${ownership_form_list[i]}</th>
                    ...
                    for (var i = 0; i < ownership_form_list.length; i++) {
                        set_canvas_element_fill_color('Parcels_legend_sign_' + i, getColor_Parcels(ownership_form_list[i]), 0.1, getColor_Parcels(ownership_form_list[i]))
                    }
                    break;
        ...
        for (y in geojson_layer._layers) {
            ...
            if (geojson_layer._layers[y].feature['geometry']['type'] == 'Polygon') {
                ...
                var layer_line = turf.lineString(geojson_layer._layers[y].feature['geometry']['coordinates'])
                    var distance = turf.pointToLineDistance(current_point_latlng, layer_line, {
                        units: 'kilometers'
                    });
...
function get_active_overlay_layers_list() {
    ...
    var active_overlay_layers_list = new Array(0)
        ...
        for (x in control_layers._layers) {
            if (
                (control_layers._layerControlInputs[x].type === "checkbox" && control_layers._layerControlInputs[x].checked === true && !control_layers._layers[x].layer._url)) // остання умова для виключення тайловий шарів
...
function get_all_user_layer_list() {
    ...
    var active_user_layers_list = new Array(0)
        ...
        for (x in control_layers._layers) {
            if (
                (control_layers._layerControlInputs[x].type === "checkbox" && !control_layers._layers[x].layer._url && control_layers._layers[x].layer.options.id.includes("user_layer", 0))) // остання умова для виключення тайловий шарів
            {
